...
const ItemMaster = () => {
  ...
  const handleDelete = async () => {
    ...
    const res = await AdminDeleteSurveyItemMaster(idToDelete, params);
    if (res.status) {
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    const res = edit?.id
      ? await AdminUpdateSurveyItemMaster(formData, params)
      : await AdminCreateSurveyItemMaster(formData, params);
    if (res.status) {
      toast.success(res.message);
      resetForm();
    } else {
          ...
          <div className="overflow-auto p-2">
            <Table className="text-body bg-new  Roles">
              <thead className="text-truncate">
                <tr>
                  {["S.No", "Name", "Rate", "Qty", "Image", "Action"].map(
                    (thead) => (
