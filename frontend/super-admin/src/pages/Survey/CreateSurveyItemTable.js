...
const CreateSurveyItemTable = () => {
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    params["action"] = edit.survey_id ? UPDATED : CREATED;
    // return console.log('form-Data', sData)
    const res = edit?.survey_id
      ? await getAdminUpdateSurvey(sData, params)
      : await getAdminCreateSurvey(sData, params);
    if (res.status) {
  ...
  return (
    <Formik
      ...
      initialValues={{
        ...
        questions: edit?.survey_questions?.map((item) => item.question) || [
          {
            item: "",
            purpose: "",
            minqty: "",
                      ...
                      <thead className="text-truncate">
                        <tr>
                          {[
                            ...
                            "User Input",
                            <>
                              <TooltipComponent align="left" title={"Add Row"}>
                                <button
                                  type="button"
                                  onClick={() =>
                                    push({
                                  ...
                                  className="shadow border-0 p-2 success-combo cursor-pointer d-align gap-1"
                                >
                            ...
                            <tr key={index}>
                              ...
                              <td>
                                <Select
                                  ...
                                  options={purposeMasterData.map((itm) => ({
                                    label: itm.name,
                                    value: itm.id,
                                  }))}
                                  value={data.purpose}
                              ...
                              <td>
                                <Form.Control
                                  type="text"
                                  name={`questions.${index}.userinput`}
