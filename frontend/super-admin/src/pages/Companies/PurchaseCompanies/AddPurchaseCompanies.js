...
const AddPurchaseCompanies = () => {
  ...
  const fetchMyCompaniesDetailsData = async () => {
    const res = await getAdminSinglePurchaseCompanies(id);
    if (res.status) {
      setEdit(res.data);
    } else {
      setEdit([]);
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    const res = edit?.company_id
      ? await updateAdminPurchaseCompanies(sData, params)
      : await addAdminMyCompanies(sData, params);
    if (res.status) {
      toast.success(res.message);
      navigate("/PurchaseCompanies");
      resetForm();
    } else {
  ...
  return (
    <Col md={12} data-aos={"fade-up"}>
      <CardComponent
        title={
          edit.company_id ? "Update Purchase Company" : "Add Purchase Company"
        }
      >
        <Formik
...
export default AddPurchaseCompanies;
