...
const PurchaseCompanies = () => {
  ...
  const fetchMyCompaniesData = async () => {
    const res = await getAdminPurchaseCompanies(search, pageSize, pageNo);
    if (res.status) {
  ...
  const viewHandler = (data) => {
    navigate(`/PurchaseCompanies/ViewCompany/${data.company_id}`, {
      state: data,
    });
  };
  ...
  return (
    <Col md={12} data-aos={"fade-up"}>
      <CardComponent
        ...
        icon={<BsPlus />}
        link={"/PurchaseCompanies/AddPurchaseCompanies/new"}
        search={true}
        searchOnChange={(e) => {
                ...
                <tr key={id}>
                  ...
                  <td>
                    <ActionButton
                      ...
                      editlink={`/PurchaseCompanies/AddPurchaseCompanies/${data.company_id}`}
                      deleteOnclick={() => {
