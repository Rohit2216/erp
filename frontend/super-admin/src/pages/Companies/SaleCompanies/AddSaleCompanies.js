...
const AddSaleCompanies = () => {
  ...
  const fetchSaleCompaniesDetailsData = async () => {
    const res = await getAdminSingleSaleCompanies(id);
    if (res.status) {
      setEdit(res.data);
    } else {
      setEdit([]);
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    params["action"] = edit.company_id ? UPDATED : CREATED;

    // return console.log('values', sData);

    const res = edit?.company_id
      ? await updateAdminSaleCompanies(sData, params)
      : await addAdminMyCompanies(sData, params);
    if (res.status) {
      toast.success(res.message);
      navigate("/SaleCompanies");
      resetForm();
    } else {
  ...
  return (
    <Col md={12} data-aos={"fade-up"}>
      <CardComponent
        title={edit.company_id ? "Update Sale Company" : "Add Sale Company"}
      >
        <Formik
          ...
          {(props) => (
            <Form onSubmit={props?.handleSubmit}>
              <Row className="g-3 align-items-center">
                ...
                <Form.Group as={Col} md="4">
                  <Form.Label>
                    Company Contact <span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Control
                    type="text"
                    maxLength={10}
                ...
                <Form.Group as={Col} md="4">
                  <Form.Label>
                    GST Treatment Type <span className="text-danger">*</span>
                  </Form.Label>
                  <Select
                ...
                <Form.Group as={Col} md="4">
                  <Form.Label>business trade name</Form.Label>
                  <Form.Control
                    type="text"
...
export default AddSaleCompanies;
