...
const RejectedComplaint = () => {
  ...
  const fetchRejectedData = async () => {
    ...
    const res = await getAdminAllRejectedComplaint(
      search,
      pageSize,
      pageNo,
      sData
      ...
      <div className="overflow-auto p-2">
        <Table className="text-body bg-new Roles">
          ...
          <tbody>
            ...
            {rejectedData?.map((complaint, idx) => (
              <tr key={idx}>
                ...
                <td>
                  {moment(complaint.status_changed_on).format("YYYY-MM-DD")}
                </td>
                <td>{complaint.status_changed_by_name}</td>
...
export default RejectedComplaint;
