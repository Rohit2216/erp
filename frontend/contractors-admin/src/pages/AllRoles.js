...
// const AllRoles = () => {
//   const { user } = useSelector(selectUser);  // Get the current logged-in user
//   const [roles, setRoles] = useState([]);
//   const [edit, setEdit] = useState({});
//   const [isLoading, setIsLoading] = useState(true);
//   const [rolePermissions, setRolePermissions] = useState([]); // Store the permissions of the user
//   const [idToDelete, setIdToDelete] = useState("");
//   const [showAlert, setShowAlert] = useState(false);
//   const [search, setSearch] = useState(0);
//   const [pageSize, setPageSize] = useState(90000000000);
...
//     // Fetch permissions based on the current logged-in user
//     const permissions = await checkPermission({ user_id: user.id });
//     setRolePermissions(permissions);  // Assuming permissions come in as an array
//   };
...
//     if (res.status) {
//       toast.success(res.message);
//       setRoles(roles.filter(role => role.id !== idToDelete));
//     } else {
//       toast.error(res.message);
//     }
//     setIdToDelete("");
...
//   useEffect(() => {
//     fetchRolesData();
//     fetchPermissions();  // Fetch user permissions when component loads
//   }, [user]);  // Re-fetch permissions when user data changes
...
//     role.name,
//     moment(role.created_at).format("DD-MM-YYYY"),
//     <Link to={`/roles/${role.id}`}>View Permissions <BsChevronDoubleRight /></Link>,
//     <ActionButton
//       deleteOnclick={() => {
//         setIdToDelete(role.id);
//         setShowAlert(true);
//       }}
//       editOnclick={() => handleEdit(role)}
//       rolePermissions={rolePermissions}  // Pass the role permissions here
//     />
//   ]);
