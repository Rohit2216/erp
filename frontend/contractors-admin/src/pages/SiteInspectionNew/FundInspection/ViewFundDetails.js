...
import {
  getApprovedFundInSiteInspection,
  getPartialFundInSiteInspection,
  getPendingFundInSiteInspection,
} from "../../../services/contractorApi";
import CardComponent from "../../../components/CardComponent";
import { printDiv } from "../../../services/helperFunctions";
import { useTranslation } from "react-i18next";

export default function ViewFundDetails() {
  ...
  const fetchTransferDetails = async () => {
    const res =
      type == "pending"
        ? await getPendingFundInSiteInspection(outletId, month?.split("-")[1])
        : type == "partial"
        ? await getPartialFundInSiteInspection(outletId, month?.split("-")[1])
        : await getApprovedFundInSiteInspection(outletId, month?.split("-")[1]);
    if (res.status) {
      setEdit(res.data);
    } else {
      setEdit([]);
  ...
  return (
    <div>
      <CardComponent title={"View Fund Inspection"} showBackButton={true}>
        {type == "approved" && (
                      ...
                      <div className="p-20 shadow rounded h-100">
                        ...
                        <ul className="list-unstyled m-2">
                          ...
                          <li>
                            ...
                            <span className="fw-bold">
                              : {data?.confirmDetails?.office?.[0]?.email}
                            </span>{" "}
                          </li>
                        </ul>
                      </div>
                        ...
                        <div className="p-20 shadow rounded h-100">
                          ...
                          <ul className="list-unstyled m-2">
                            ...
                            <li>
                              {t("contact person email")}
                              <span className="fw-bold">
                                : {data?.confirmDetails?.site?.[0]?.email}
                              </span>{" "}
                            </li>
                          </ul>
                        </div>
                                  ...
                                  <tr key={idx}>
                                    ...
                                    {type == "approved" && (
                                      <>
                                        ...
                                        <td>
                                          ₹ {itm?.total_site_approved_amount}
                                        </td>
                                      </>
                                    )}
