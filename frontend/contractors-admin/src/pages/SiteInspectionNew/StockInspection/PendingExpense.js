...
const PendingExpense = () => {
  ...
  const fetchExpenseRequestData = async () => {
    const res = await getSiteExpensePendingRequest(
      salesAreaId,
      RegionalId,
      outletId,
      pageSize,
  ...
  const fetchSalesArea = async () => {
    const res = await getAllSalesByIdForSiteInspection(0);
    if (res.status) {
  ...
  const fetchOutletArea = async () => {
    const res = await getAllOutletByIdForSiteInspection(0);
    if (res.status) {
  ...
  const fetchRoOffice = async () => {
    const res = await getAllRegionalByIdForSite(0);
    if (res.status) {
                            ...
                            <tr key={id1}>
                              ...
                              <td>
                                <ActionButton
                                  ...
                                  approveOnclick={() =>
                                    navigate(
                                      `/approve-site-expense-inspection`,
                                      {
                                        state: {
                                          outletId: data?.outlet?.[0]?.id,
                                          month: data?.month,
