...
import {
  ...
  getAllSalesArea,
  getAllSalesAreaForOfficeInspection,
  postAssignComplaint,
...
const PendingExpense = ({ getValue }) => {
  ...
  const showSalesAreaApi = async () => {
    const res = await getAllSalesAreaForOfficeInspection();
    if (res.status) {
      setSaData(res.data);
    } else {
      setSaData([]);
    ...
    <>
      <div className="table-scroll p-2">
        <div className="w-100 mb-3 d-align justify-content-end gap-3">
          ...
          {/* <Link
            to={`/office-expense/food-expense-punch`}
            className="social-btn-re text-none w-auto d-flex align-items-center danger-combo px-3"
          >
            Food Expense Punch
          </Link> */}
          <Select
                  ...
                  <tr>
                    ...
                    <td>
                      <ActionButton
                        hideDelete={"d-none"}
                        hideEdit={"d-none"}
                        eyelink={`/office-expense/office-complaints-on-outlet/${e.outlet_id}?type=${getValue}`}
                      />
                    </td>
                  </tr>
                ))}
