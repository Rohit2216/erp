...
const PoViewDetails = () => {
  ...
  const fetchSingleDetailsData = async () => {
    const res =
      type === "purchase-order"
        ? await getSinglePurchaseOrderById(id, pageSize, pageNo, search)
        : await getSinglePurchaseOrderById(id, pageSize, pageNo, search);
    if (res.status) {
      setViewDetails(res.data);
      setPageDetail(res.data.purchase_order_item.pageDetails);
    } else {
      setViewDetails({});
      setPageDetail({});
    }
        ...
        >
          {type === "purchase-order" ? (
            <Row className="g-3 py-1">
              ...
              {ViewDetails?.purchase_order_item?.data.length > 0 && (
                <Col md={12}>
                  <div className="p-20 mb-5 shadow rounded h-100">
                    ...
                    <div className="mt-2">
                      <Table className="table-sm table Roles">
                        ...
                        <tbody>
                          {ViewDetails.purchase_order_item.data.map(
                            (item, index) => (
                              <>
                                <tr key={index}>
                      ...
                      <ReactPagination
                        ...
                        nextClassName={
                          pageSize == pageDetail?.total
                            ? ViewDetails.purchase_order_item.data.length - 1 <
                              pageSize
                              ? "danger-combo-disable pe-none"
                              : "success-combo"
                            : ViewDetails.purchase_order_item.data.length <
                              pageSize
                            ? "danger-combo-disable pe-none"
                            : "success-combo"
                        }
