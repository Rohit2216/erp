...
import {
  getAllRescheduledStockTransfer,
  getAllRescheduledTransfer,
} from "../../services/contractoApi2";
import ReactPagination from "../../components/ReactPagination";
import { useTranslation } from "react-i18next";

export default function RescheduledStockTransfer() {
  ...
  const getRescheduledTransferFund = async () => {
    const res = await getAllRescheduledStockTransfer(pageSize, pageNo);
    if (res.status) {
                ...
                results.map((data, id1) => (
                  <tr key={id1}>
                    ...
                    <td className={`fw-bolder text-danger`}>
                      {data?.request_stock_quantity}
                    </td>
                    <td className={`fw-bolder text-green`}>
                      {data?.approve_stock_quantity}
                    </td>
