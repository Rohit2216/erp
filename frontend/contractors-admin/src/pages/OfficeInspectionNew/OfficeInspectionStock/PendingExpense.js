...
import {
  getAllOutletByIdForOfficeInspection,
  getAllRegionalByIdForOffice,
  getAllSalesByIdForOfficeInspection,
  getOfficeExpenseRequest,
...
const PendingExpense = () => {
  ...
  const fetchSalesArea = async () => {
    const res = await getAllSalesByIdForOfficeInspection(0);
    if (res.status) {
  ...
  const fetchOutletArea = async () => {
    const res = await getAllOutletByIdForOfficeInspection(0);
    if (res.status) {
  ...
  const fetchRoOffice = async () => {
    const res = await getAllRegionalByIdForOffice(0);
    if (res.status) {
        ...
        <Card className="card-bg">
          ...
          >
            <Tab
              ...
              title={t("office stock inspection")}
            />
            <Tab className="ms-auto" title={t("Pending Expense")}>
              {activeTab == "2" && (
                <>
