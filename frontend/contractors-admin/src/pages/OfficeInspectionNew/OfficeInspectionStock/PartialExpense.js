...
export default function PartialExpense() {
  ...
  const fetchSalesArea = async () => {
    const res = await getAllSalesByIdForOfficeInspection(1);
    if (res.status) {
  ...
  const fetchOutletArea = async () => {
    const res = await getAllOutletByIdForOfficeInspection(1);
    if (res.status) {
  ...
  const fetchRoOffice = async () => {
    const res = await getAllRegionalByIdForOffice(1);
    if (res.status) {
