...
import {
  ...
  getOfficeExpenseRequestForApproved,
...
export default function ApprovedExpense() {
  ...
  const fetchSalesArea = async () => {
    const res = await getAllSalesByIdForOfficeInspection(2);
    if (res.status) {
  ...
  const fetchOutletArea = async () => {
    const res = await getAllOutletByIdForOfficeInspection(2);
    if (res.status) {
  ...
  const fetchRoOffice = async () => {
    const res = await getAllRegionalByIdForOffice(2);
    if (res.status) {
