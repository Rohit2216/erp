...
import {
  ...
  getOfficeExpenseRequestForPartial,
  getOfficeFundRequestForPartial,
...
export default function PartialFund() {
  ...
  const fetchSalesArea = async () => {
    const res = await getAllSalesForFundInOffice(1);
    if (res.status) {
  ...
  const fetchOutletArea = async () => {
    const res = await getAllOutletForFundInOffice(1);
    if (res.status) {
  ...
  const fetchRoOffice = async () => {
    const res = await getAllRegionalForFundInOffice(1);
    if (res.status) {
