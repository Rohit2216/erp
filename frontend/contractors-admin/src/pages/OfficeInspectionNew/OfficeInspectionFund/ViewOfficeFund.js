...
import {
  getOfficeFundRequestByOutletIdForApproved,
...
export default function ViewOfficeFund() {
  ...
  const fetchTransferDetails = async () => {
    const res =
      type == "pending"
        ...
        : await getOfficeFundRequestByOutletIdForApproved(
            outletId,
            month.split("-")[1]
          );
    if (res.status) {
  ...
  return (
    <div>
      <CardComponent title={"View Office fund Punch"} showBackButton={true}>
        {type == "approved" && (
