...
const db = makeDb();
const {companyValidation, checkPositiveInteger, emailValidation} = require('../helpers/validation');
const { StatusCodes } = require('http-status-codes');
const {getGstDetailsByCompanyId, getAdminDetailsById} = require('../helpers/general');


const addSaleCompany = async (req, res) => {

    try 
    {
        const {name, email, contact, mobile, address, primary_contact_person, primary_contact_mobile, primary_contact_email, designation, department, company_website, gst_treatment_type, business_legal_name, business_trade_name, pan_number, gst_details, place_of_supply} = req.body;  

        const {error, value} = saleCompanyFiledValidated.validate(req.body);

        if(error) return res.status(400).json({status: false, message: error.message});

        const createdBy = req.user.user_id;
        const insertQuery = `INSERT INTO sale_companies (name, email, contact, mobile, address, primary_contact_person, primary_contact_mobile, primary_contact_email, designation, department, company_website, gst_treatment_type, business_legal_name, business_trade_name, pan_number, gst_number, place_of_supply, billing_address, shipping_address, created_by) VALUES ('${name}', '${email}', '${contact}', '${mobile}', '${address}', '${primary_contact_person}', '${primary_contact_mobile}', '${primary_contact_email}', '${designation}', '${department}', '${company_website}', '${gst_treatment_type}', '${business_legal_name}', '${business_trade_name}', '${pan_number}', '${gst_number}', '${place_of_supply}', '${billing_address}', '${shipping_address}', '${createdBy}')`;
        ...
        db.query(insertQuery, (err, result) => {
            ...
            {
                res.status(200).json({status: true, message: "Sale company added"})
            }
            else
            {
...
const getSaleCompanies = async (req, res) => {
    ...
    try 
    {
        ...
        const countSelectQuery = `SELECT COUNT(*) as total FROM companies WHERE (company_type = ? OR company_type = ?) AND is_deleted = ? AND companies.is_superadmin_company = ? `
        constTotalLength = await db.query(countSelectQuery, [process.env.SALE_COMPANY,  process.env.BOTH_COMPANY, process.env.INACTIVE_STATUS, process.env.VALUE_ZERO]);
        ...
        queryParams.unshift(process.env.VALUE_ZERO, process.env.SALE_COMPANY, process.env.BOTH_COMPANY, process.env.INACTIVE_STATUS);
...
const getSaleCompanyById = async (req, res) => {
    ...
    try 
    {
        ...
        const queryResult = await db.query(selectQuery, [process.env.SALE_COMPANY, id]);
...
const editSalesCompany = async (req, res) => {
...
const updateSalesCompany = async (req, res) => {
...
const removeSalesCompanyById = async (req, res) => {
...
module.exports = {addSaleCompany, getSaleCompanies, getSaleCompanyById, editSalesCompany, updateSalesCompany, removeSalesCompanyById}
