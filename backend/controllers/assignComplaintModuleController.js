...
const assignComplaintModule = async (req, res) => {
    try {
...
const getAssignComplaintModuleOnUserId = async (req, res) => {
    try {
...
const assignMultipleComplaintModule = async (req, res) => {
    try {
        ...
        if (module_ids.length > process.env.VALUE_ZERO) {
            ...
            for (var i = 0; i < module_ids.length; i++) {
                ...
                const assign_date = moment(new Date()).format('YYYY-MM-DD');
                const assign_by = req.user.user_id;
                const assignData = { module_id, module_type, assign_to, is_future_date_visible, assign_date, assign_by, assign_for };
